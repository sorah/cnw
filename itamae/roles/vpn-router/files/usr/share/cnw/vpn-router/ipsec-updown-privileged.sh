...
if [ $# -gt 0 ]; then
  exec env "$@" /usr/share/cnw/vpn-router/ipsec-updown-privileged.sh
fi
...
case "${PLUTO_VERB}" in
  up-client|up-client-v6)
    ...
    VTI_RTB="$(grep "^conn ${PLUTO_CONNECTION}" /etc/ipsec.conf|sed -r 's/^.*#.*rtb=([^,]+?)(,.+$|$)/\1/')"
    if [ -n "${VTI_RTB}" ]; then
      ip rule add iif "${VTI_IF}" table "${VTI_RTB}"
    fi

    sysctl -w "net.ipv4.conf.${VTI_IF}.disable_policy=1"
    sysctl -w "net.ipv6.conf.${VTI_IF}.disable_policy=1"
